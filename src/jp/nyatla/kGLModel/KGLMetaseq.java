...
//	    }
	...
	private KGLPoint[] readVertex(int num, multiInput br, float scale) throws Exception
	{
		...
		System.err.println("MQOファイル　フォーマットエラー（Object>vertex）[" + line+ "]");
		throw e;
	    }
	...
	throws Exception {
		...
		System.err.println("MQOファイル　フォーマットエラー（Object>Bvertex）[" + line+ "]");
	...
	private Face[] readFace(multiInput br) throws Exception
	{
		...
		System.err.println("MQOファイル　フォーマットエラー（Object>face）"+ e.getMessage() + "[" + line + "]");
		throw e;
	    }
	    if (qf.size() == 0)
		return null;
     ...
     * @return 描画用マテリアル情報
     */
    private GLMaterial makeMats(material mqomat, int i_mqomat, objects mqoObjs,KGLPoint[] vn) {
	...
	ret.texID = texPool.getGLTexture(mqomat.data.tex, mqomat.data.aplane,
		false);
	if (apv.size() == 0)
	    return null;
	uvValid &= (ret.texID != 0);
	...
	ret.interleaveFormat = GL.GL_N3F_V3F;
	if (uvValid && colValid)
	    ret.interleaveFormat = GL.GL_T2F_C4F_N3F_V3F;
	else if (uvValid)
	    ret.interleaveFormat = GL.GL_T2F_N3F_V3F;
	else if (colValid)
	    ret.interleaveFormat = GL.GL_C4F_N3F_V3F;
	ret.interleaved = ByteBuffer.allocateDirect((ret.vertex_num * 3 + // 頂点の数＊３（ｘｙｚ）
		ret.vertex_num * 3 + // 頂点法線の数（＝頂点の数）
		((uvValid) ? ret.vertex_num * 2 : 0) + // ＵＶの数（頂点の数ｘ２）
		((colValid) ? ret.vertex_num * 4 : 0))
		* // 頂点色の数（頂点の数ｘ４）
		Float.SIZE);
	ret.interleaved.order(ByteOrder.nativeOrder());
	ret.interleaved.position(0);
	for (int v = 0; v < ret.vertex_num; v++) {
	    if (uvValid) {
		ret.interleaved.putFloat(wft[v].U());
		ret.interleaved.putFloat(wft[v].V());
	    }
	    if (colValid) {
		ret.interleaved.putFloat(wfc[v].R());
		ret.interleaved.putFloat(wfc[v].G());
		ret.interleaved.putFloat(wfc[v].B());
		ret.interleaved.putFloat(wfc[v].A());
	    }
	    ret.interleaved.putFloat(wfn[v].X());
	    ret.interleaved.putFloat(wfn[v].Y());
	    ret.interleaved.putFloat(wfn[v].Z());
	    ret.interleaved.putFloat(wfv[v].X());
	    ret.interleaved.putFloat(wfv[v].Y());
	    ret.interleaved.putFloat(wfv[v].Z());
	}
	if (mqomat.data.col != null) {
     ...
     * @return 描画用オブジェクト情報
     */
    private GLObject makeObjs(material mqoMats[], objects mqoObjs)
    {
	GLObject ret = null;
	...
	for (int m = 0; m < mqoMats.length; m++) {
	    mr = makeMats(mqoMats[m], m, mqoObjs, vn);
	    if (mr != null) {
		mats.add(mr);
	    }
	}
	...
	if (!isUseVBO)
	    return ret;
	ret.VBO_ids = new int[ret.mat.length];
	gl.glGenBuffers(ret.mat.length, ret.VBO_ids, 0);
	for (int m = 0; m < ret.mat.length; m++) {
	    ret.mat[m].interleaved.position(0);
	    gl.glBindBufferARB(GL.GL_ARRAY_BUFFER_ARB, ret.VBO_ids[m]);
	    gl.glBufferDataARB(GL.GL_ARRAY_BUFFER_ARB, ret.mat[m].interleaved
		    .capacity(), ret.mat[m].interleaved, GL.GL_STATIC_DRAW_ARB);
	}
	/* */
...
//    }
     ...
     * 頂点配列バッファを使用するかどうか
     */
    protected KGLMetaseq(GL in_gl, KGLTextures in_texPool,ContentProvider i_provider,float scale, boolean isUseVBO)
    {
	super(in_gl, in_texPool, i_provider,scale, isUseVBO);
//	targetMQO = in_moq;
	this.file_provider=i_provider;
	material mats[] = null;
	...
	try {
	    fis = i_provider.CreateMainStream().getInputStream();
		...
		if (chankName[0].trim().toUpperCase().equals("MATERIAL")) {
			...
			for (int m = 0; m < mats.length; m++) {
			    ...
			    // System.out.println(mats[m].toString()) ;
			}
		    } catch (Exception mat_e) {
			System.err.println("MQOファイル　Materialチャンク読み込み例外発生 "+ mat_e.getMessage());
		...
		try {
			...
			glo = makeObjs(mats, object);
			if (glo != null){
			    globjs.add(glo);
			}
		    }
		}catch (Exception obj_e) {
		    System.err.println("MQOファイル　Object[" + chankName[1]+ "]チャンク読み込み例外発生 " + obj_e.toString());
